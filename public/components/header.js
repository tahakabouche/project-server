import { isAuthenticated, logout } from "../js/modules/auth.js";
import { get } from "../js/modules/api.js";
import { getCart } from "../js/modules/utils.js";

class MyHeader extends HTMLElement {
  constructor() {
    super();
    if (isAuthenticated()) {
      this.innerHTML = `
    <header class="nav-bar">
        <a class="nav__logo logo-a" href="#">
          <h2 class="logo">taha</h2>
        </a>    
        <nav class="nav__links hideOnMobile">
            <ul>
                <li> 
                  <a class="nav-link" href="#">Home</a>
                </li>
          
                <li>
                  <a class="nav-link" href="#">Shop</a>
                </li>
                 <li>
                    <div class="dropdown" data-dropdown>
                            <button class="dropdown-link nav-link" data-dropdown-button>
                               <span>Tops</span>
                               <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M480-344 240-584l56-56 184 184 184-184 56 56-240 240Z"/></svg>      
                            </button>
                            <div class="dropdown-menu">
                                <ul class="nav-dropdown-menu-options">
                                  <li>
                                    <a href="/index.html?category=Jackets">Jackets</a> 
                                  </li>
                                  <li>
                                    <a href="/index.html?category=T-shirts" >T-shirts</a>   
                                  </li>
                                  <li>
                                    <a href="/index.html?category=Sweatshirts">Sweatshirts</a>   
                                  </li>
                                  <li>
                                    <a href="/index.html?category=Hoodies">Hoodies</a>   
                                  </li>
                      
                                  </ul>
                            </div>
                        </div>
                  </li>
                  <li>
                      <div class="dropdown" data-dropdown>
                            <button class="dropdown-link nav-link" data-dropdown-button>
                              <span>Bottoms</span>
                              <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M480-344 240-584l56-56 184 184 184-184 56 56-240 240Z"/></svg>      
                            </button>
                            <div class="dropdown-menu">
                                <ul class="nav-dropdown-menu-options">
                                  <li>
                                    <a href="/?category=Jeans">Jeans</a> 
                                  </li>
                                  <li>
                                    <a href="/?category=Sweatpants" >Sweatpants</a>   
                                  </li>
                                  <li>
                                    <a href="/?category=Shorts">Shorts</a>   
                                  </li>
                                </ul>
                            </div>
                        </div>
                  </li>
                  <li>
                  <a class="nav-link" href="#">Contact</a>
                </li>
            </ul>
        </nav>
        <div class="nav__search hideOnMobile" >
          <img id="search-icon" src="assets/icons/search.svg" />
          <input class="search-input" type="text" placeholder="Search for products..." />
        </div>
        <ul class="nav__icons">
            <li>
              <a href="/cart-page.html" id="cart-icon-a">
                <p id="nav-cart-total">DA0,0</p>
                <img class="cart-icon" src="assets/icons/cart.svg" alt="" />
                <div id="nav-cart-items-number" class="cart-items-number">0</div>
              </a>
            </li>
            <li>
              <a class="menu-btn" >
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M120-240v-80h720v80H120Zm0-200v-80h720v80H120Zm0-200v-80h720v80H120Z"/></svg>
              </a>
            </li>
            <li class="hideOnMobile">
              <a id="login-btn" href="/manage-products.html" id="">
                <svg style="margin-top: 0.15rem;" class="account-icon" width="30.5" height="30.5" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M11 0.875C8.99747 0.875 7.0399 1.46882 5.37486 2.58137C3.70981 3.69392 2.41206 5.27523 1.64572 7.12533C0.879387 8.97543 0.678878 11.0112 1.06955 12.9753C1.46023 14.9393 2.42454 16.7435 3.84055 18.1595C5.25656 19.5755 7.06066 20.5398 9.02471 20.9305C10.9888 21.3211 13.0246 21.1206 14.8747 20.3543C16.7248 19.5879 18.3061 18.2902 19.4186 16.6251C20.5312 14.9601 21.125 13.0025 21.125 11C21.122 8.3156 20.0543 5.74199 18.1562 3.84383C16.258 1.94567 13.6844 0.877978 11 0.875ZM6.45969 17.4284C6.98195 16.7143 7.66528 16.1335 8.45418 15.7331C9.24308 15.3327 10.1153 15.124 11 15.124C11.8847 15.124 12.7569 15.3327 13.5458 15.7331C14.3347 16.1335 15.0181 16.7143 15.5403 17.4284C14.2134 18.3695 12.6268 18.875 11 18.875C9.37323 18.875 7.78665 18.3695 6.45969 17.4284ZM8.375 10.25C8.375 9.73082 8.52896 9.22331 8.8174 8.79163C9.10583 8.35995 9.5158 8.0235 9.99546 7.82482C10.4751 7.62614 11.0029 7.57415 11.5121 7.67544C12.0213 7.77672 12.489 8.02673 12.8562 8.39384C13.2233 8.76096 13.4733 9.22869 13.5746 9.73789C13.6759 10.2471 13.6239 10.7749 13.4252 11.2545C13.2265 11.7342 12.8901 12.1442 12.4584 12.4326C12.0267 12.721 11.5192 12.875 11 12.875C10.3038 12.875 9.63613 12.5984 9.14385 12.1062C8.65157 11.6139 8.375 10.9462 8.375 10.25ZM17.1875 15.8694C16.4583 14.9419 15.5289 14.1914 14.4688 13.6737C15.1444 12.9896 15.6026 12.1208 15.7858 11.1769C15.9689 10.2329 15.8688 9.25583 15.498 8.36861C15.1273 7.4814 14.5024 6.72364 13.702 6.19068C12.9017 5.65771 11.9616 5.37334 11 5.37334C10.0384 5.37334 9.09833 5.65771 8.29797 6.19068C7.49762 6.72364 6.87275 7.4814 6.50198 8.36861C6.13121 9.25583 6.0311 10.2329 6.21424 11.1769C6.39739 12.1208 6.85561 12.9896 7.53125 13.6737C6.4711 14.1914 5.54168 14.9419 4.8125 15.8694C3.89661 14.7083 3.32614 13.3129 3.1664 11.8427C3.00665 10.3725 3.2641 8.88711 3.90925 7.55644C4.55441 6.22578 5.5612 5.10366 6.81439 4.31855C8.06757 3.53343 9.5165 3.11703 10.9953 3.11703C12.4741 3.11703 13.9231 3.53343 15.1762 4.31855C16.4294 5.10366 17.4362 6.22578 18.0814 7.55644C18.7265 8.88711 18.984 10.3725 18.8242 11.8427C18.6645 13.3129 18.094 14.7083 17.1781 15.8694H17.1875Z" fill="black"/>
                </svg>
              </a>
            </li>    
        </ul>
        <ul class="main-side-bar">
          <li>
            <a class="nav-link close-menu-btn">
              <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"/></svg>
            </a>
          </li>
          <li>
            <div class="nav__search" >
              <img id="search-icon" src="assets/icons/search.svg" />
              <input class="search-input" type="text" placeholder="Search for products..." />
            </div>
          </li>
          <li><a class="nav-link" href="#">Home</a></li>
          <li><a class="nav-link" href="#">Shop</a></li>
          <li>
                  <button class="flex-row-div-2" onclick=toggleSubMenu(this)>
                    <span class="nav-link">Tops</span>
                    <svg class="filters-arrow down"class="filters-arrow" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M480-344 240-584l56-56 184 184 184-184 56 56-240 240Z"/></svg>
                  </button>
                  <ul class="collapsible expanded">
                    <div class="nav-collapsible">
                       <li>
                        <a href="/index.html?category=Jackets">Jackets</a> 
                      </li>
                      <li>
                        <a href="/index.html?category=T-shirts" >T-shirts</a>   
                      </li>
                      <li>
                        <a href="/index.html?category=Sweatshirts">Sweatshirts</a>   
                      </li>
                      <li>
                        <a href="/index.html?category=Hoodies">Hoodies</a>   
                      </li>
                    </div>
                  </ul>
          </li>
          <li>
            <button class="flex-row-div-2" onclick=toggleSubMenu(this)> 
               <span class="nav-link">Bottoms</span>   
               <svg class="filters-arrow down" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M480-344 240-584l56-56 184 184 184-184 56 56-240 240Z"/></svg>
            </button>
            <ul class="collapsible expanded">
              <div class="nav-collapsible">
                <li>
                  <a href="/?category=Jeans">Jeans</a> 
                </li>
                <li>
                  <a href="/?category=Sweatpants" >Sweatpants</a>   
                </li>
                <li>
                  <a href="/?category=Shorts">Shorts</a>   
                </li>
              </div>
            </ul>
          </li>
          <li><a class="nav-link" href="#">Contact</a></li>
          <li><a class="nav-link menu-logout-btn">Logout</a></li>
        </ul>
      </header>
    `;
    } else {
      this.innerHTML = `
      <header class="nav-bar">
          <a class="nav__logo logo-a" href="#">
            <h2 class="logo">taha</h2>
          </a>    
         <nav class="nav__links hideOnMobile">
            <ul>
                <li> 
                  <a class="nav-link" href="#">Home</a>
                </li>
               
                <li>
                  <a class="nav-link" href="#">Shop</a>
                </li>
                 <li>
                    <div class="dropdown" data-dropdown>
                            <button class="dropdown-link nav-link" data-dropdown-button>
                               <span>Tops</span>
                               <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M480-528 296-344l-56-56 240-240 240 240-56 56-184-184Z"/></svg>                            </button>
                            <div class="dropdown-menu">
                                <ul class="nav-dropdown-menu-options">
                                  <li>
                                    <a href="/index.html?category=Jackets">Jackets</a> 
                                  </li>
                                  <li>
                                    <a href="/index.html?category=T-shirts" >T-shirts</a>   
                                  </li>
                                  <li>
                                    <a href="/index.html?category=Sweatshirts">Sweatshirts</a>   
                                  </li>
                                  <li>
                                    <a href="/index.html?category=Hoodies">Hoodies</a>   
                                  </li>
                      
                                  </ul>
                            </div>
                        </div>
                  </li>
                  <li>
                      <div class="dropdown" data-dropdown>
                            <button class="dropdown-link nav-link" data-dropdown-button>
                              <span>Bottoms</span>
                              <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M480-528 296-344l-56-56 240-240 240 240-56 56-184-184Z"/></svg>                            </button>
                            <div class="dropdown-menu">
                                <ul class="nav-dropdown-menu-options">
                                  <li>
                                    <a href="/?category=Jeans">Jeans</a> 
                                  </li>
                                  <li>
                                    <a href="/?category=Sweatpants" >Sweatpants</a>   
                                  </li>
                                  <li>
                                    <a href="/?category=Shorts">Shorts</a>   
                                  </li>
                                </ul>
                            </div>
                        </div>
                  </li>
                   <li>
                  <a class="nav-link" href="#">Contact</a>
                </li>
            </ul>
        </nav>
          <div class="nav__search hideOnMobile" >
            <img id="search-icon" src="assets/icons/search.svg" />
            <input class="search-input" type="text" placeholder="Search for products..." />
          </div>
          <ul class="nav__icons">
              <li>
                <a href="/cart-page.html" id="cart-icon-a">
                  <p id="nav-cart-total">DA0,0</p>
                  <img class="cart-icon" src="assets/icons/cart.svg" alt="" />
                  <div id="nav-cart-items-number" class="cart-items-number">0</div>
                </a>
              </li>
              <li class="hideOnMobile">
                <a id="login-btn" class="hideOnMobile" href="/login.html">
                  <button class="btn btn--login">Login</button>
                </a>
              </li>
              <li>
                <a class="menu-btn">
                  <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M120-240v-80h720v80H120Zm0-200v-80h720v80H120Zm0-200v-80h720v80H120Z"/></svg>
                </a>
              </li>
          </ul>
          <ul class="main-side-bar">
            <li>
              <a class="nav-link close-menu-btn">
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"/></svg>
              </a>
            </li>
            <li>
              <div class="nav__search" >
                <img id="search-icon" src="assets/icons/search.svg" />
                <input class="search-input" type="text" placeholder="Search for products..." />
              </div>
            </li>
            <li><a class="nav-link" href="#">Home</a></li>
            <li><a class="nav-link" href="#">Shop</a></li>
            <li><a class="nav-link" href="#">Contact</a></li>
            <li><a class="nav-link" href="/sign-up.html">Sign Up</a></li>
            <li><a class="nav-link" href="/login.html">Log In</a></li>
          </ul>
        </header>
      `;
    }
  }

  connectedCallback() {
    this.sideBar = this.querySelector(".main-side-bar");
    this.navCartTotal = this.querySelector("#nav-cart-total");
    this.navCartItemsNumber = this.querySelector("#nav-cart-items-number");
    this.menuBtn = this.querySelector(".menu-btn");
    this.closeMenuBtn = this.querySelector(".close-menu-btn");

    this.menuBtn.addEventListener("click", () => this.showSideBar());
    this.closeMenuBtn.addEventListener("click", () => this.hideSideBar());

    if (isAuthenticated()) {
      this.menuLogoutBtn = this.querySelector(".menu-logout-btn");
      this.menuLogoutBtn.addEventListener("click", logout);
    }

    this.updateCartLogo();
  }

  static get observedAttributes() {
    return ["total", "items-count"];
  }

  attributeChangedCallback(name, oldValue, newValue) {
    if (name === "total")
      this.navCartTotal.innerText = `DA${newValue.toLocaleString("en-US")}`;
    if (name === "items-count") this.navCartItemsNumber.innerText = newValue;
  }

  set cartTotal(total) {
    this.setAttribute("total", total);
  }

  set cartItemsCount(count) {
    this.setAttribute("items-count", count);
  }

  async updateCartLogo() {
    if (isAuthenticated()) {
      const response = await get("auth/me");
      this.setAttribute("total", response.data.cartTotal);
      this.setAttribute("items-count", response.data.cart.length);
    } else {
      const cart = getCart();
      const cartTotal = cart.reduce((total, item) => {
        return total + item.product.price * item.quantity;
      }, 0);
      console.log(cartTotal);
      console.log(cart);
      this.setAttribute("total", cartTotal);
      this.setAttribute("items-count", cart.length);
    }
  }

  showSideBar() {
    this.sideBar.style.display = "flex";
  }

  hideSideBar() {
    this.sideBar.style.display = "none";
  }
}

customElements.define("my-header", MyHeader);
